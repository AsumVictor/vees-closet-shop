...
function ProductSearchPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        let res = await axios(
          `${server}product/search?q=${searchQuery}&page=${currentPage}&sort=${sortQuery}`
        );
        if (res.data.success) {
    ...
    window.scrollTo(0, 0);
  }, [currentPage, sortQuery, searchQuery]);
  ...
  return (
    <div className="py-20 w-full">
       <Helmet>
        <title>Shop high quality {searchQuery} - Vees closet </title>
        <meta
          name="description"
          content={`Explore our wide range of high-quality ${searchQuery} for every need. Find the perfect ${searchQuery} at Vees closet.`}
        />
        <meta
          name="keywords"
