...
function ShopCategoryPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        let res = await axios(
          `${server}product/products-by-category?category=${params.name}&page=${currentPage}&sort=${sortQuery}`
        );
        if (res.data.success) {
  ...
  return (
    <div className="py-20 w-full">
      <Helmet>
        <title>Shop high quality {params.name} - Vees closet </title>
        <meta
          name="description"
          content={`Explore our wide range of high-quality ${params.name} for every need. Find the perfect ${params.name} at Vees closet.`}
        />
        <meta
          name="keywords"
