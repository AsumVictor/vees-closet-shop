...
function ProductDetails() {
    ...
    useEffect(() => {
        ...
        const getRealtedProduct = async () => {
            try {
                let res = await axios(`${server}product/related-products?name=${params.name}`);
                if (res.data.success) {
    ...
    const addToCart = async () => {
        ...
        try {
            ...
            if (product.hasVariations) {
                ...
                if (res.isEmptyKeys) {
                    setSubmissionError(
                        `Select a variant(s) for the product. ${res.keys.map((key) => `{${key}}`)}`
                    );
                    return;
                }
            }
            ...
            <div className=" mt-10 grid md:grid-cols-2 grid-cols-1 gap-5">
                ...
                <div className="py-2 flex flex-col px-5 md:px-2">
                    ...
                    <p className="mt-3 font-semibold flex flex-row gap-2  text-[20px]">
                        <span className="price text-primary-600 ">₵ {product.actual_price}</span>
                        {product.base_price && (
                        ...
                        <div className="w-full py-1 mt-5 flex flex-col gap-3">
                            {product.variations.map((variant) => (
                                <div className="" key={variant._id}>
                                    <label className=" capitalize" htmlFor={variant.variation._id}>
