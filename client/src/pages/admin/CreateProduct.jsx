...
function CreateProduct() {
    ...
    const handleVariationChangeRemove = (index, data) => {
        setProduct((prev) => {
            ...
            newVarState[index].selected_values = newVarState[index].selected_values.filter(
                (i) => i !== data
            );
            return {
                ...prev,
    ...
    const VariationChangeAdd = (variation) => {
        let duplicate = product.variations.some((i) => i.variation === variation._id);
    ...
    const createProduct = async (e) => {
        ...
        try {
            ...
            setLoading(true);
            let res = await axios.post(`${server}product/create-product`, {
                ...product,
                images: images,
            ...
            setLoading(false);
            errMsg = error?.response?.data?.message ? error.response.data.message : error.message;
            toast.error(errMsg);
        }
    };
    ...
    return (
        <form onSubmit={createProduct} className=" relative w-full px-3 bg-white py-5">
            <Link
                to={'..'}
                relative="path"
            ...
            <div className="w-full grid grid-cols-12 mt-5 gap-y-10">
                <div className="w-full py-1  col-span-full 1000px:col-span-8 1300px:col-span-7">
                    <h1 className="text-xl font-medium text-center ">Product information</h1>
                    ...
                    <div className="w-full mt-5">
                        <div className="w-full flex flex-row gap-2">
                            ...
                            />
                            <label htmlFor="isVaries">Does this product has variations ?</label>
                        </div>
                    </div>
                    ...
                    {isVarError && (
                        <p className="text-red-600 py-2 px-3 text-center font-medium text-[16px] mb-2">
                            ** You have check that product has variations but did not add variants.
                            Add variations to the product and make sure all variants have value(s).
                        </p>
                    )}
                    <div className="w-full mt-5">
