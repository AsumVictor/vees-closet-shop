...
function Cart({ data, quantityChangeHandler, removeFromCartHandler }) {
    ...
    return (
        <div className="w-full py-2 grid grid-cols-10 gap-x-3">
            ...
            >
                <img src={data.images[0].url} alt={data.name} className="w-full h-full" />
            </Link>
            <Link
                to={`/products/${productUrl}`}
            ...
            >
                ...
                <p className="mt-5 flex flex-row gap-2">
                    <span className="text-wine_primary font-bold  800px:text-xl ">{`GH₵ ${data.priceWithDiscount.toFixed(
                        2
                    )}`}</span>
